...
def generate_launch_description():
    ...
    namespace = LaunchConfiguration("namespace")  # 默认为空
    params_file = LaunchConfiguration(
        "params_file"
    )  # 默认为config/nav2_raw_params.yaml
    use_sim_time = LaunchConfiguration("use_sim_time")  # 默认为True
    autostart = LaunchConfiguration("autostart")  # 默认为True
    use_respawn = LaunchConfiguration("use_respawn")  # 默认为False
    log_level = LaunchConfiguration("log_level")  # 默认为warn
    ...
    # 如果提供的参数文件中没有 slam_toolbox 的参数，我们必须移除 params_file 配置，否则它将自动传递给 slam_toolbox，并且不会加载默认文件。
